...
grid <- seq(
  quantile(endline$rs0_z, 0.05, na.rm = TRUE),
  quantile(endline$rs0_z, 0.95, na.rm = TRUE),
  length.out = 100
)

mod_regime <- fit_het_model_for_figure(endline, "t1_rs_index", "t0_rs_index", "rs0_z")
mod_trust <- fit_het_model_for_figure(endline, "t1_trust_foreign", "t0_trust_foreign", "rs0_z")
...
p <- ggplot(plot_df, aes(x = z, y = estimate, color = treatment, fill = treatment)) +
  ...
  labs(
    x = "Baseline regime support (z)",
    y = "Marginal treatment effect",
    title = "Heterogeneous Effects by Baseline Regime Support"
  ) +
  paper_theme()

save_figure(p, "fig_heterogeneity_regime_support_t1", width = 9, height = 4.8)
message("Wrote figures/fig_heterogeneity_regime_support_t1.pdf")
