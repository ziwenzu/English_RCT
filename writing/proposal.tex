...
The final materials bank contains 89 unique readings: 24 pro-China political pieces, 24 anti-China political pieces, 17 apolitical China-related pieces, and 24 non-China neutral controls. Unlike earlier drafts, the realized bank is source-diversified rather than concentrated in only a few elite newspapers. It draws from \emph{Reuters}, \emph{AP News}, \emph{The Guardian}, \emph{The Economist}, \emph{The Washington Post}, \emph{The New York Times}, and smaller numbers from \emph{The Wall Street Journal}, \emph{BBC Travel}, \emph{CNN Travel}, and \emph{National Geographic}. This diversification reduces the risk that respondents simply react to a single outlet label instead of to the substantive content of the reading.
...
As a post-curation content-validity check, I score the finalized plain-text corpus using article-level tone measures. This exercise is conceptually similar to press-level validity checks in related work on Western coverage of China, such as the evidence in Chen and Han's \emph{Economic Journal} study showing systematically more negative English-language Western coverage than \emph{China Daily} on an embedding-based scale. In the current curation process, this text analysis serves two purposes. First, it verifies that the overtly political pools point in the intended direction, with pro-China materials using more favorable language on average than anti-China materials. Second, it screens the benchmark pools for unintended affective loading. In practice, this screen is important because lifestyle journalism, travel writing, and food features can look ``neutral'' politically while still carrying strongly positive descriptive language. For that reason, the final benchmark pools are curated not only for topical neutrality but also for approximate lexical neutrality relative to the political pools.

As a separate non-substantive comparability check, I also measured article-level word count, average sentence length, average word length, and Flesch--Kincaid grade. These diagnostics show that the pools are reasonably close on sentence length and general reading difficulty, but not perfectly matched on article length. The sharpest imbalance is that the apolitical-China and non-China benchmark pools tend to be longer and somewhat easier than the hard-news political pools, which is a predictable consequence of mixing travel, food, museum, and culture features with political reporting. When translated into the realized 24-reading arm bundles, those differences narrow but do not disappear entirely. I therefore treat textual balance as an explicit design diagnostic rather than an unexamined assumption, and I report these checks alongside the main content-validity evidence.
...
\begin{figure}[H]
\centering
\scalebox{0.7}{\input{figures/figure_flowchart.tex}}
