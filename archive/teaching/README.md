...
### Current Scope of Behavioral Measures

The current manuscript uses module openings, reading minutes, quiz performance, and weekly in-app evaluations of interest, credibility, and desire for similar content as its main behavioral measures. Those weekly evaluations are recorded after each completed reading, including neutral filler lessons, so the design already contains reading-level neutral benchmarks for placebo-style checks. The course also included short recap videos, and the teaching-data structure retains a placeholder for video-viewing information. Those video measures are not used in the current manuscript analyses and should be treated as reserved for a later harmonized release rather than as part of the present behavioral-results battery. The current data release does not yet include a separate article-specific placebo-question battery for non-China control articles beyond those standard weekly evaluations. Any placebo-style weekly analysis that requires additional article-specific follow-up prompts for control content should therefore be treated as pending a later data update rather than as available in the present files.
...
`study_id`, `content_id` (1–24), `week` (1–12), `slot_wk` (1–2), `article_id`, `bank`, `sched_valence`, `wk_open`, `wk_comply`, `wk_read_min`, `wk_vid_pct` (reserved for a later harmonized video-view measure; not used in the current manuscript analyses), `wk_quiz_score` (0–100), `wk_rate_interest`, `wk_rate_cred`, `wk_rate_similar` (all 0–100). Because these weekly measures are recorded for political and neutral readings alike, low-dose slot-1 filler weeks and common slot-2 fillers can already be used for placebo-style diagnostics. The current release does not yet append any additional article-specific placebo-question responses for non-China control articles beyond those standard weekly measures.
