...
* Limit ourselves to one observation per month
...
twoway (sc coef time_to_treat, connect(line) mlab("")) ///
	...
	xtitle("Months relative to Blockage") ytitle("Estimated coefficients") ///
	xtick(-1, add) xlabel(-1, add) legend(off) graphregion(fcolor(white))

	graph save "$tempDir/Figure4a.gph", replace
	graph export "$OUT_main/Figure4a.pdf", replace

	graph close 

	
keep coef time_to_treat ci_top ci_bottom
export delimited "$auxDir/Figure4a_estimates.csv", replace
...
twoway (sc coef time_to_treat, connect(line) mlab("")) ///
	...
	graph save "$tempDir/Figure4b.gph", replace
	graph export "$OUT_main/Figure4b.pdf", replace

	graph close 

keep coef time_to_treat ci_top ci_bottom
export delimited "$auxDir/Figure4b_estimates.csv", replace
